...
namespace Exoa.Designer
{
    public class UIFloorMapMenu : GenericLeftMenu
    {
        ...
        private void ToggleMenu(bool animate = true)
        {
            ...
            {
                if (animate)
                    menuItems[i].createBtn.transform.DOLocalMoveX(isCreateMenuOpen ? menuItems[i].xPos : createButton.transform.localPosition.x, .5f);
                else menuItems[i].createBtn.transform.localPosition = menuItems[i].createBtn.transform.localPosition.SetX(
                    (isCreateMenuOpen ? menuItems[i].xPos : createButton.transform.localPosition.x));
        ...
        public void OnDuplicateUIItem(DataModel.FloorMapItem s, DataModel.FloorMapItemType type)
        {
            ...
            scrollRect.DOVerticalNormalizedPos(0, .5f);
        }
